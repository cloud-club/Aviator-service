...
func (n Network) getNetworkInterfaceList(dto *network.RequestBodyNetworkInterfaceList) (*http.Response, error) {
	...
	url := fmt.Sprintf("%s/getNetworkInterfaceList", common.BaseUrl)
	//regionCode=%s&subnetName=%s,&networkInterfaceNoList.1=%s&networkInterfaceName=%s&networkInterfaceStatusCode=%s&ip=%s&secondaryIpList.1=%s&instanceNo%s&isDefault=%s&deviceName=%s&serverName=%s",
